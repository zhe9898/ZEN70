...
async def pin_login(
    ...
    async def _handle_failure(detail: str):
        ...
        if count >= PIN_RATE_LIMIT_MAX:
            ...
            raise zen(
                CODE_TOO_MANY,
                f"连续失败 {PIN_RATE_LIMIT_MAX} 次，已触发 60 秒锁定防爆破",
                status.HTTP_429_TOO_MANY_REQUESTS,
            )
...
async def update_ai_preference(
    ...
    log_auth(
        ...
        request_id(request),
        username=username,
        detail=f"changed_to_{req.preference}",
    )
...
async def invite_webauthn_register_begin(
    ...
    if not token_data_str:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="凭证已失效或不存在")
