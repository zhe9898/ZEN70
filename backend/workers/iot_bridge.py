...
MAX_RETRIES = 3
DLQ_STREAM_KEY = "zen70:iot:stream:dlq"
IDEMPOTENCY_SET_PREFIX = "zen70:iot:idempotency:"
            ...
            if "state" in data:
                ...
                if self.loop:
                    asyncio.run_coroutine_threadsafe(
                        self._broadcast_sse_state(device_id, state),
                        self.loop
                    )
                else:
                    ...
                    for message_id, data in messages:
                        ...
                        try:
                            ...
                            if retries >= MAX_RETRIES:
                                logger.critical(f"🔥 [{trace_id}] [IoT Bridge] 超过最大重试次数，打入 DLQ 告警: {message_id}")
                                # 直接放进 DLQ 等待人工干预，极限界定 10000 避免死信轰爆内存
                                await self.redis.xadd(DLQ_STREAM_KEY, data, maxlen=10000, approximate=True)
                                await self.redis.xack(REDIS_STREAM_KEY, CONSUMER_GROUP, message_id)
                            else:
                                # 留着不 ACK，由待处理消息回收逻辑 (XPENDING) 重新分配
                                pass 
